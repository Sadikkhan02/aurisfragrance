...
const Login = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(currentState === 'Sign Up'){
        ...
        const respose = await axios.post(backendUrl + '/api/user/login', {email, password});
        if(respose.data.success){
          setToken(respose.data.token);
          localStorage.setItem('token', respose.data.token)
        }else{
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <input onChange={(e)=> setPassword(e.target.value)} value={password} type="password" className='w-full px-3 py-2 border border-gray-800 ' placeholder='**********' required/>
